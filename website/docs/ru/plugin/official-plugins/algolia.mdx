...
Для настройки переведённого текста на разных языках Rspress предоставляет следующий переведённый текст, который можно импортировать с помощью оператора `import`.
...
Rspress по умолчанию предоставляет перевод на китайский язык, и вы можете настроить переведенный текст на различных языках с помощью параметра `locales`.
