...
Значение конфигурационного параметра `search.searchHooks` — это путь к файлу. В этом файле должны экспортироваться нужные хуки (например, `onSearch`), которые реализуют вашу логику кастомизации поиска во время выполнения. Такой файл мы будем называть **модулем `searchHooks`**.
