...
При использовании абсолютных путей корневой директорией считается [`docs`](/api/config/config-basic#root). Если в проекте включена [мультиязычность](/guide/default-theme/i18n) или [мультиверсионность](/guide/default-theme/multi-version), параметр [`markdown.link.autoPrefix`](/api/config/config-build#markdownlinkautoPrefix) автоматически добавляет нужные префиксы, поэтому папку с языком в ссылках можно опускать.
